...
<table width="100%" border="1">
    ...
    $sql = "SELECT r.refundID, u.Fname AS UserName, ni.bname AS ItemName, r.reason
            FROM refund r
            JOIN payment p ON r.payID = p.payID
            JOIN user u ON p.userID = u.userID
            JOIN newitem ni ON p.bcode = ni.bcode";
    ...
    if ($result) {
        while ($row = $result->fetch_assoc()) {
            echo "<tr>
                    ...
                    <td>" . $row["ItemName"] . "</td>
                    <td>" . $row["reason"] . "</td>
                    <td><a href='confirm.php'>✔️</a></td>
                    <td><a href='deleterefund.php?refund=" . $row["refundID"] . "'>❌</a></td>
                </tr>";
        }
