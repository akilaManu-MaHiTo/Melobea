					...
					if($result->num_rows>0){
						while($row=$result->fetch_assoc()){
							echo"<tr>
									...
									<td>".$row["udate"]."</td>
									<td>".$row["ucountry"]."</td>
									<td><a href='edituseditemsdb.php?code=$row[ucode]&brand=$row[ubrand]&type=$row[utype]&description=$row[udep]&price=$row[uprice]&date=$row[udate]&country=$row[ucountry]'>🖊️</a></td>
                                    <td><a href='deleteuseditem.php?code=$row[ucode]'>❌</a></td>
                                      </tr>";
                            }
						}	
				?>
