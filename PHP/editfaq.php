<head>
        ...
        <!--Style Sheet-->
            <link rel="stylesheet" href="Style/newitem.css">
        <!-- End Style Sheet-->
    ...
    <body>
        ...
        <?php
            ...
            $sql = "SELECT f.userID, s.Fname, f.que, f.ans, f.faqid
                    FROM faq f
                    JOIN user s ON f.userID = s.userID";

            $result = $mysqli->query($sql);

            if ($result->num_rows > 0) {
                echo '<table>';
                echo '<tr><th>User ID</th><th>First Name</th><th>Question</th><th>FAQ ID</th><th>Answer</th><th>Edit</th><th>Delete</th></tr>';
                while ($row = $result->fetch_assoc()) {
                    echo "<tr>";
                    echo "<td>" . $row["userID"] . "</td>";
                    echo "<td>" . $row["Fname"] . "</td>";
                    echo "<td>" . $row["que"] . "</td>";
                    echo "<td>" . $row["faqid"] . "</td>";
                    echo "<td>" . $row["ans"] . "</td>";
                    echo "<td><a href='editfaqform.php?faqid=" . $row["faqid"] . "&prashne=" . $row["que"] . "&answer=" . $row["ans"] . "'>🖊️</a></td>";
                    echo "<td><a href='faqeditdb.php?faq=" . $row["faqid"] . "'>❌</a></td>";
                    echo "</tr>";
                }
                echo '</table>';
            }
